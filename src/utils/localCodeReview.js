export function generateLocalCodeReview({ code = "", challenge = null }) {
  ...
  if (!trimmed) {
    ...
    suggestions.push("Start by implementing the required function from the starter code.");
  }

  if (/\bpass\b/.test(trimmed)) {
    findings.push("`pass` is still present, so the function is incomplete.");
    suggestions.push("Replace `pass` with real logic and return the expected value.");
  }

  if (!/\breturn\b/.test(trimmed)) {
    findings.push("No `return` statement detected in your code.");
    suggestions.push("Ensure the function returns the final output expected by tests.");
  } else {
    positives.push("A return statement is present, which is required for most challenge functions.");
  }

  if (description.includes("set") && !/\bset\s*\(/.test(trimmed)) {
    findings.push("Challenge asks for set conversion, but `set(...)` was not detected.");
    suggestions.push("Use `set(items)` to remove duplicates efficiently.");
  }

  if (/for\s+\w+\s+in\s+\w+/.test(trimmed) && /append\s*\(/.test(trimmed)) {
    suggestions.push("If you only need unique items, direct set conversion is simpler than manual loop + append.");
  }
  ...
  const lines = [
    ...
    "",
    "#### Findings",
    ...(findings.length ? findings.map((f) => `- ${f}`) : ["- No critical structural issues detected from static checks."]),
    "",
    "#### Suggestions",
    ...(suggestions.length ? suggestions.map((s) => `- ${s}`) : ["- Add edge-case tests and verify behavior with empty/invalid inputs."]),
    "",
    "#### Complexity",
    `- ${complexity}`,
  ];
