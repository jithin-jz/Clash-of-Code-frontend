        ...
        <div className="flex gap-4 flex-wrap justify-center items-center">
          {achievements.map(({ achievement, unlocked_at }) => {
            ...
            return (
              ...
              >
                <div className={`absolute inset-full blur-md opacity-0 group-hover:opacity-40 transition-opacity rounded-full ${colors.bg}`} />
                <IconComponent size={15} strokeWidth={2.5} className={`${colors.text} drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]`} />
              </Motion.button>
            )
          })}
        </div>
          ...
          >
            ...
            >
              <div className="relative inline-flex mb-6">
                <div className={`absolute inset-0 blur-2xl opacity-20 ${(CATEGORY_COLORS[selectedAchievement.category] || CATEGORY_COLORS.challenge).bg}`} />
                <div className={`relative w-24 h-24 rounded-2xl ${(CATEGORY_COLORS[selectedAchievement.category] || CATEGORY_COLORS.challenge).bg} border ${(CATEGORY_COLORS[selectedAchievement.category] || CATEGORY_COLORS.challenge).border} flex items-center justify-center`}>
                  {(() => {
                    const IconDetail = ICON_MAP[selectedAchievement.icon] || Trophy;
                    return <IconDetail 
                      size={40} 
                      className={(CATEGORY_COLORS[selectedAchievement.category] || CATEGORY_COLORS.challenge).text} 
                    />;
                  })()}
                </div>
              </div>
                ...
                {selectedAchievement.unlocked_at && (
                  <div className="flex items-center gap-1.5 text-neutral-600">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-wider">
                      Earned {new Date(selectedAchievement.unlocked_at).toLocaleDateString(undefined, {
